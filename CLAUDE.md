...
The reusable package is intentionally small: `src/pytorch_pcgrad` should expose PCGrad only. Multi-MNIST training code lives under `examples/multi_mnist/` and is not part of the published module surface. It composes a shared representation network (`MultiLeNetR`) and two task-specific output heads (`MultiLeNetO`) from `examples/multi_mnist/net/lenet.py`, creates the Multi-MNIST dataset via `examples/multi_mnist/data/multi_mnist.py`, wraps an Adam optimizer with `PCGrad`, computes left/right digit losses separately, and calls `optimizer.pc_backward(losses)` before `optimizer.step()`.
